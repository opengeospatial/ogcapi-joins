...
The __data joining operations__ operation set is represented by the requirements classes: "<<rc_data_joining,data joining>>" and "<<rc_join_delete,join delete>>". The implementation of the "data joining" requirements class is mandatory, and the implementation of the "join delete" is optional for this operation set.
...
The JSON encoding format support is mandatory, and the HTML encoding support is recommended for the service responses in the __discovery operations__ and __data joining operations__ operation sets. 
