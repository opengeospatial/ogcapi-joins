...
. If the server specifies the maximum value for the `limit` parameter, execute the test <<ats_data_joining_collections-collectionid-keys-keyfieldid-get-op, /conf/data-joining/collections-collectionid-keys-keyfieldid-get-op>> with a `limit` parameter value that exceeds the maximum value.
