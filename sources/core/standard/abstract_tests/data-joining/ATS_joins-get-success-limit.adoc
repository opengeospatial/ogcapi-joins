...
. If the server specifies the maximum value for the `limit` parameter, execute the test <<ats_data_joining_joins-get-op, /conf/data-joining/joins-get-op>> with a `limit` parameter value that exceeds the maximum value.
