...
<<http_status_codes,HTTP status codes>> lists the main HTTP status codes that clients should be prepared to receive. This includes support for specific security schemes or URI redirection. In addition, other error situations may occur in the transport layer outside of the server.
...
The return status codes described in <<http_status_codes,HTTP status codes>> do not cover all possible conditions. See <<rfc7231,IETF RFC 7231>> for a complete list of HTTP status codes.
...
The landing page response is based on the https://schemas.opengis.net/ogcapi/common/part1/1.0/openapi/schemas/landingPage.yaml[Landing Page Schema], originally defined in the <<OGC19-072,OGC - API - Common - Part 1>>.
...
The schema for the conformance response is based on the https://schemas.opengis.net/ogcapi/common/part1/1.0/openapi/schemas/confClasses.yaml[Conformance Declaration Schema] (from <<OGC19-072,OGC API - Common - Part 1>>).
...
The response is based on the https://github.com/opengeospatial/ogcapi-common/blob/master/collections/openapi/schemas/common-geodata/collections.yaml[Collections Schema] (from <<OGC20-024,OGC - API - Common - Part 2>>).
...
include::./requirements/data-joining/REC_collections-get-success-describedby.adoc[]
...
The Collection response is based on the https://github.com/opengeospatial/ogcapi-common/blob/master/collections/openapi/schemas/common-geodata/collectionDesc.yaml[collectionDesc Schema] (from <<OGC20-024,OGC - API - Common - Part 2>>).
...
If the parameter `collectionId` does not exist on the server, the status code of the response will be `404`. (see <<http_status_codes,Table 4>>).
...
include::./requirements/data-joining/PER_collections-collectionid-keys-keyfieldid-get-op-key-param.adoc[]

include::./requirements/data-joining/PER_collections-collectionid-keys-keyfieldid-get-op-limit-param.adoc[]
...
If the parameter `collectionId` does not exist on the server, the status code of the response will be `404`. (see <<http_status_codes,HTTP status codes>>).

If the parameter `keyFieldId` does not exist on the server, the status code of the response will be `404`. (see <<http_status_codes,HTTP status codes>>).
...
include::./requirements/data-joining/PER_joins-get-op-limit-parameter.adoc[]

include::./requirements/data-joining/PER_joins-get-op-datetime-parameter.adoc[]
...
The input data file can either be uploaded to the server with the query, represented by the requirements class "<<rc_file_uploading_with_query, Input File Upload>>" or referenced via URL links, represented by the requirements class "<<rc_file_referencing_with_url, File Referencing with URL>>".

The core module contains support for the CSV format for the input data files, defined in the requirements class "<<rc_input_csv, CSV File Input>>". The CSV files must have a column that contains the key values that will be used in the join operation and a separate column for each joined attribute.
...
The operation is required to support the GeoJSON format for the joined data, represented by the requirements class "<<requirements_class_output-geojson, GeoJSON Output for Joined Data>>" and it may support direct GeoJSON output, represented by the requirements class "<<requirements_class_output-geojson-direct, Direct GeoJSON Output for Joined Data>>". In addition, the operation may support any other output format for the joined data.
...
include::./requirements/data-joining/PER_joins-post-op-additional-output-formats.adoc[]

include::./requirements/data-joining/REQ_joins-post-op-additional-output-formats.adoc[]

==== Response

include::./requirements/data-joining/REQ_joins-post-success.adoc[]

include::./requirements/data-joining/REQ_joins-post-success-input-data-file-csv-multiple-keys.adoc[]

include::./requirements/data-joining/REQ_joins-post-success-input-data-file-csv-attribute-names.adoc[]
...
If the parameter `joinId` does not exist on the server, the status code of the response will be `404`. (see <<http_status_codes,HTTP status codes>>).
...
The input data files can be either uploaded to the server with the query, represented by the requirements class "<<rc_file_uploading_with_query, Input File Upload>>" or referenced via URL links, represented by the requirements class "<<rc_file_referencing_with_url, File Referencing with URL>>".
...
* The CSV format for the right dataset, defined in the requirements class "<<rc_input_csv, CSV File Input>>".
* The GeoJSON format for the left dataset, defined in the requirements class "<<rc_input_geojson, GeoJSON File Input>>".
