...
part:: If the server implements __data joining operations__, it SHALL support the HTTP POST operation at path `{root}/joins`.
...
part:: The Form Data parameters of the query are listed in a <<req_data_joining_joins-post-op-form-data-parameters,Table 5>>:
====